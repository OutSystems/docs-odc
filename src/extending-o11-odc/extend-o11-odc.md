...
On the other hand, there are scenarios where extending with ODC may not be the right path. Follows some examples where you should continue developing in O11:

* **Extensions of existing O11 apps:** Adding few screens or UI components to an existing O11 application (for example, adding a new section to an existing Customer Portal), or making minor additions to a tightly coupled O11 system.
...
Start extending your portfolio using [data interoperability](data-interoperability/data-interop.md) and [logic interoperability](logic-interoperability/logic-interop.md).
