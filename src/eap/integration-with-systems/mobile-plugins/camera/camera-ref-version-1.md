---
summary: (Version 1) Explore the Camera Plugin functionalities in OutSystems Developer Cloud (ODC), including client actions, web blocks, and error handling.
tags: plugin development, mobile camera integration, error handling, mobile application development, outsystems api
locale: en-us
guid: 5bace646-4be5-4d45-94b7-8aae53a71812
...
# Camera plugin version 1 reference

<div class="info" markdown="1">

This reference article applies to Camera Plugin version 1.x. If you are using Camera Plugin version 2.x, refer to the [Camera Plugin version 2 reference article](camera-ref.md).
...
| SaveToPhotoAlbum | Input | Boolean | Sets if the media captured can be saved in the device’s gallery. Not available on PWAs. |
| CameraDirection | Input | Structure | Select the front or back camera as default when taking a new picture. Default value: BackCamera. Not available on Android. |
| AllowEdit | Input | Boolean | Sets if the media captured must be sent to an edit screen right after the capturing. |
| AllowMultiplePictures | Input | Boolean | In a PWA context, if set to True, the user can take pictures in bulk when this action is used in a cycle. Using this functionality might affect the picture quality. |
...
Opens a screen to edit a given picture. Not available on PWAs.

| Parameter | Type | Data Type | Description |
| - | - | - | - |
| InputImage | Input | Binary Data | Sets the URI of the photo to be edited. |
...
Allow users to choose a picture, video, or both, directly from their gallery. Not available on PWAs.

| Parameter | Type | Data Type | Description |
| - | - | - | - |
| MediaType | Input | [MediaType Entity](#mediatype-static-entity) | Sets which media type is available for selection, if photo, video, or both. |
| AllowEdit | Input | Boolean | Sets if the media captured must be sent to an edit screen right after the capturing. |
...
DEPRECATED, should use the new TakePicture client action instead.
...
| CameraDirection | Input | Structure | Select the front or back camera as default when taking a new picture. Default value: BackCamera. Not available on Android. |
| AllowEdit | Input | Boolean | Sets if the media captured must be sent to an edit screen right after the capturing. |
| AllowMultiplePictures | Input | Boolean | Sets if the user can select multiple pictures in PWA. |
...
DEPRECATED. Use the ChooseFromGallery client action instead. The exception is PWAs, where ChooseFromGallery is not available.
...
| CameraDirection | Input | Structure | Select the front or back camera as default when taking a new picture. Default value: BackCamera. Not available on Android. |
| AllowEdit | Input | Boolean | Sets if the media captured must be sent to an edit screen right after the capturing. |
| AllowMultiplePictures | Input | Boolean | Not used. |
| ImageCaptured | Output | Structure | Returns the captured image and its information. |
| Success | Output | Boolean | Indicates if the action was successful ('True') or not ('False'). |
| Error | Output | Error | Displays detailed information of an error, if applicable. |
...
A button that triggers the action ChooseFromGallery, allowing users to select an image from their device. Not available on PWA. For PWA, you may use the  **DEPRECATED_ChooseImage** block.
...
| Resolution | Text | Returns the resolution of the media in pixels. |
