---
summary: File Viewer plugin in OutSystems Developer Cloud (ODC) opens remote and app resource files in mobile apps, with native previews on iOS.
tags:
  - Android
  - Capacitor
  - Cordova
...
![Screenshot of a PDF file open in the Android File Viewer Plugin, showing sample.pdf rendered in a native viewer](images/file-viewer-preview-sa.png "Android File Viewer PDF Preview")
...
1. Select the file you added under **Resources**, and:
    ...
    * Select **Deploy to Target Directory** in the **Deploy Action** list.
    ...
    ![Screenshot showing the resources properties settings for the File Viewer Plugin in ODC Studio](images/resources-file-viewer-odcs.png "Resources Properties for File Viewer")
...
![Screenshot of the logic flow for using the File Viewer Plugin in a mobile app](images/logic-file-viewer-odcs.png "Logic for File Viewer Plugin")
...
From File Viewer plugin 2.1.3 onwards, when this happens, **CheckFileViewerPlugin** returns **IsAvailable** as **True**, with **Warning.WarningCode** set to **OS-PLUG-FLVW-0002**, meaning the installed native plugin predates version 2.0.0. You can continue using all client actions (deprecated and non-deprecated).

<div class="info" markdown="1">

File Viewer plugin versions between 2.0.0 and 2.1.2 returned **False** for **IsAvailable** in this scenario, so only the deprecated client actions worked.

This was fixed in version 2.1.3 of the File Viewer plugin. Update to this version, especially if you can't have your users all update to the new native build.
