...
| Type in SQL queries | OutSystems Data Type | Microsoft SQL Server | MySQL | Oracle | PostgreSQL | Salesforce | SAP OData | Snowflake |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| BOOLEAN | Boolean | BIT<br/>DECIMAL(1,0)<br/>NUMERIC(1,0) | BIT(1)<br/>BOOL<br/>TINYINT(1) | NUMBER(1, 0) | BIT(1)<br/>BOOL | BIT | | BOOLEAN |
...
| DECIMAL(p, s) | Decimal<br/>Integer (if p < 10 and s = 0)<br/>Long Integer (if p = 10-18 and s = 0)<br/>Text (if p > 28 or s > 8) | DECIMAL(p, s) | DECIMAL(p, s) | NUMBER(p, s) | MONEY<br/>NUMERIC(p, s) | | DECIMAL | NUMBER(p, s) <br/>NUMERIC(p, s) |
| DECIMAL(38, 0) | Text | | | | | | | TINYINT [NUMBER(38,0)]<br/>SMALLINT [NUMBER(38,0)]<br/>INT [NUMBER(38,0)]<br/>INTEGER [NUMBER(38,0)]<br/>BYTEINT [NUMBER(38,0)]<br/>BIGINT [NUMBER(38,0)] |
| NUMERIC<br/>(p, s) | Decimal<br/>Integer (if p < 10 and s = 0)<br/>Long Integer (if p = 10-18 and s = 0)<br/>Text (if p > 28 or s > 8) | NUMERIC(p, s) | NUMERIC(p, s) | NUMBER(p, s) | | | | |
| FLOAT<br/>REAL | Text | REAL | FLOAT | FLOAT<br/>REAL | FLOAT4 | | | |
| DOUBLE | Text | FLOAT | REAL | | FLOAT8 | FLOAT | | FLOAT<br/>FLOAT4<br/>FLOAT8<br/>REAL<br/>DOUBLE<br/>DOUBLE PRECISION |
| CHAR | Text | CHAR<br/>NCHAR | CHAR | CHAR<br/>NCHAR | BPCHAR | | | |
| VARCHAR | Text | DATETIMEOFFSET<br/>NTEXT<br/>TEXT<br/>UNIQUEIDENTIFIER<br/>VARCHAR<br/>XML | ENUM<br/>JSON<br/>SET<br/>TINYTEXT<br/>UUID<br/>VARCHAR | CLOB<br/>LONG<br/>NCLOB<br/>NVARCHAR<br/>ROWID<br/>UROWID<br/>VARCHAR | ENUM<br/>JSON<br/>PG_LSN<br/>TEXT<br/>UUID<br/>VARCHAR<br/>XML | UUID<br/>VARCHAR | UUID<br/>VARCHAR | TIMESTAMP_TZ<br/>CHAR [VARCHAR(1)]<br/>CHARACTER [VARCHAR(1)]<br/>VARCHAR(p)<br/>STRING [VARCHAR(65536)]<br/>TEXT [VARCHAR(65536)] |
| BINARY | Binary Data | BINARY | BINARY | BLOB | BYTEA | | | BINARY [BINARY(65536)]<br/>VARBINARY [BINARY(65536)] |
| VARBINARY | Binary Data | IMAGE<br/>VARBINARY | BLOB<br/>LONGBLOB<br/>MEDIUMBLOB<br/>TINYBLOB<br/>VARBINARY | RAW | | | | |
| DATE | Date | DATE | DATE<br/>YEAR | DATE | DATE | DATE | DATE | DATE |
| TIME | Text | TIME | TIME | | TIME | TIME | TIME | TIME |
| TIMESTAMP | DateTime | DATETIME<br/>DATETIME2<br/>SMALLDATETIME | DATETIME<br/>TIMESTAMP | TIMESTAMP | TIMESTAMP | TIMESTAMP | | TIMESTAMP_NTZ<br/>TIMESTAMP_LTZ |
...
The Snowflake timestamp with timezone data type (TIMESTAMP_TZ) is mapped to Text in ODC.
Values are returned in the format `yyyy-MM-dd HH:mm:ss.SSS ZZ` (e.g. `2020-03-12 01:02:03.123 +0200`).
