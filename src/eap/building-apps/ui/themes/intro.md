...
![Screenshot showing an example of an application theme in a user interface design](images/application-theme-odcs.png "Application Theme Screenshot")
...
* **Public** - By default, it's set to **No**, so other apps can't share. To share the same look and feel between apps, developers need to create a [theme library](#theme-libraries-theme-library).
...
* **Style Sheet** -  Define custom CSS to style your theme. You can write CSS directly in the stylesheet editor to customize colors, fonts, spacing, and other visual properties. Alternatively, click **Open Theme Editor** to use a visual interface that allows you to configure basic styling options like primary color, font, and font size without writing CSS. The Theme Editor automatically generates the corresponding CSS code in the stylesheet. This gives you flexibility to use either approach or combine both methods for your theme customization.
  
* **Icon Library** – Choose which icon library your theme uses. This determines the icon set available when you add Icon widgets to screens that use this theme. You can select one of the following values:
    * **Phosphor 2.0** for a modern, extensive range of icons. Default for new apps.
    * **Font Awesome 4.0** for compatibility with existing or legacy apps. Default for existing apps.
    * Library inherited from the base theme
  
  Choose the icon library that aligns with your app’s visual style and helps maintain a consistent look across all UI elements. For detailed information, refer to [Icons](../icons/intro.md).
