...
1. To create your library using the class library template, run the following code. Give the project a name, such as `ConsumeSOAPExample`. The command creates a class library targeting the .NET version installed in the [prerequisites](README.md#prerequisites).

    `dotnet new classlib --language "C#" -o ConsumeSOAPExample`
