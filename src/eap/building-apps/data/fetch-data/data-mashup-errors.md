...
Errors may occur when executing aggregates with multiple sources and if the execution plan consumes too much memory. Queries that use data from different sources may require a large amount of memory and CPU to process the required data, depending on the query complexity, logic, and data volume. For other differences between Test Query and runtime results, refer to [understand Test Query results](test-query-runtime.md).
