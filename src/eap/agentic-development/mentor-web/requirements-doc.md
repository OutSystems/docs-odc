...
While short prompts are good for generating straightforward apps and making quick changes, a requirement document provides a structured specification for more complex apps. This "scaffold and refine" workflow lets you provide detailed requirements that ODC can interpret consistently, producing a more accurate blueprint for your review.

Use a well-structured document to define a detailed data model, establish specific roles and permissions, and lay out your screen requirements from the start. Requirement documents are one of two input methods in the [app generation workflow](how-it-works.md).
...
Structure your requirement document so ODC has the context required to generate accurate data models, screens, and business logic. Use consistent sections to explain the app purpose, user roles, data relationships, and interface expectations. Review the sample documents later in this guide to see the structure in practice.
...
When defining entity attributes, include a data type so ODC generates the correct fields and validations. Use the table below to match each attribute to the appropriate type.
...
Use these examples as patterns when drafting each portion of your requirement document. They demonstrate how to translate business intent into precise statements that can be interpreted accurately.
...
When requesting dashboards, state the elements and aggregation types to generate.
...
Include the following types of content in your requirement document and avoid content that agentic development can't process.
...
* **Personal data**: Don't include personally identifiable information (PII) such as real names, email addresses, phone numbers, or government IDs. Use placeholder or fictional data for all examples and sample records.
* **Implementation code**: Don't include code snippets or technical implementation instructions
* **Screenshots or embedded images**: They aren't processed
* **Complex tables**: Use clear lists and clear text instead
