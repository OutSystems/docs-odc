...
locale: en-us
summary: Manage services installed in self-hosted Kubernetes clusters by OutSystems Developer Cloud (ODC), focusing on security, secrets management, and observability components.
...
When you set up ODC self-hosted, OutSystems installs a set of services into your Kubernetes cluster. These services make sure your self-hosted stages can run applications securely, reliably, and in full integration with the rest of the ODC platform.  
...
![OutSystems Self-hosted configuration screen on the Install services step, listing Container registry, HashiCorp Vault, Flux CD, NATS, SeaweedFS, Prometheus, OpenTelemetry, FluentBit, Gloo Edge, Istio, and Runtime Services as installed.](images/install-services-shc.png "Self-hosted configuration wizard showing installed platform services")
...
**Container registry** The container registry stores the application images generated during the build process. These images are versioned and kept available so they can be deployed into your cluster when needed. Depending on your setup, this is either an OutSystems-managed in-cluster registry or a custom registry you provide. Refer to [Set up the custom OCI registry for self-hosted stages](sh-registry.md) for details on both options.
...
## Messaging and coordination {#messaging}
...
## Networking and traffic management {#networking}
...
## OutSystems runtime services {#runtime-services}
...
Do not uninstall or delete any OutSystems runtime services. Doing so does not guarantee a clean state and can cause conflicts if you trigger a new installation afterward. To remove a self-hosted stage, refer to [Remove a stage](install-sh.md#remove-stage).

</div>

![Diagram of a self-hosted Kubernetes cluster showing CI/CD services, system storage, networking and traffic, secrets management, monitoring and telemetry, messaging with NATS, and OutSystems services connected to test and production runtime stages.](images/sh-cluster-services-diag.png "Architecture diagram of services installed in a self-hosted cluster")
