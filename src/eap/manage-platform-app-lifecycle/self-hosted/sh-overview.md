...
locale: en-us
summary: Understand the architecture of ODC Self-hosted, runtime stages on customer infrastructure, Kubernetes orchestration, outbound connectivity, and security requirements.
...
* **Control over infrastructure**: Customers control infrastructure components, including Kubernetes clusters and PostgreSQL databases, with full responsibility for patching and updates. Your Kubernetes distribution orchestrates these clusters, automating tasks such as scaling, resource allocation, and failover.
...
![High-level diagram showing OutSystems Cloud hosting the Portal, Dev runtime stage, OutSystems services, and Dev database, connected to a self-hosted Kubernetes cluster that runs Test and Prod runtime stages with their own databases, OutSystems services, an APM tool, and an identity provider.](images/self-hosted-diag.png "High-level architecture of ODC Self-hosted")
...
## Detailed architecture view {#detailed-architecture}
...
A container registry sits at the boundary between cloud and self-hosted, acting as the handoff point. When a build completes in the cloud, the resulting container image is synced to the container registry, making it available for deployment into your self-hosted stages. The registry is either an OutSystems-managed in-cluster registry (the default on OpenShift) or a customer-provided external registry (available on all distributions).

**Your self-hosted cluster** contains both the OutSystems runtime services and the supporting services OutSystems installs alongside them. The runtime services are the same ones that power the Development stage in the cloud, but here they run under your infrastructure, supporting your Test and Production stage apps. The supporting services are grouped by function:

* **CI/CD services** (Container registry, Flux CD): receive the synced container images and deploy them into the correct runtime stage. The container registry is either OutSystems-managed (in-cluster, OpenShift default) or customer-provided (external, all distributions).
...
* **Container registry**: for custom registry configurations, a customer-managed OCI-compliant registry is required before you start setup. Refer to [Set up the custom OCI registry for self-hosted stages](sh-registry.md) for requirements and configuration details.
